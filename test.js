...
const convertToDiscussion = (obj) => {
  ...
  const checked = document.createElement('p'); // img 
  checked.textContent = obj.answer ? "❤" : "❌"; // 삼항연산자 // src 이미지 링크를 넣어준다.! // 폰트어썸 아이콘 있는 사이트
...
// form 요소는 submit 동작이 일어나면 자동으로 새로고침
form.addEventListener('submit', (event) => {
    event.preventDefault(); // submit을 해도 새로고침이 되지 않는다.
    // 하나의 객체를 만들어서, convertToDiscussion 함수에 넣어서 li로 만든 다음 ul요소에 append
    const newDiscussion = {
        id: "unique value",
        createdAt: new Date(),
        title: title.value,
