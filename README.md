...
> localStorage를 이용해서 새로고침해도 데이터가 유지되게 구현한 것!

<br>

### 구현하고 싶었는데 하지 못한 아쉬운 기능

> 시간과 기력이 더 있었다면 답변 여부에 따른 필터링 기능을 구현해봐도 좋았을 것 같다.

<br>

### 도움을 받고 싶은 부분

> 댓글 작성하기 버튼을 누르면 폼 형식이 보이게 되는 기능을 `display: none;` 을 이용해서 구현했는데, 이 방법을 사용하면 transition이 적용되지 않아 화면이 다소 부자연스러워 보인다. `visibility: hidden`을 이용하면 transition은 적용 가능했지만 요소가 보이지만 않을 뿐 자리를 그대로 차지하고 있어서 원하는 대로 구현되지 않았다. 속시원한 방법이 있으면 좋겠다!
