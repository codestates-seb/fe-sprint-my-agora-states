...
const convertToDiscussion = (obj) => {
  ...
  info.textContent = `${obj.author} / 
  ${new Date(obj.createdAt).toLocaleString('ko-KR')}`;
  discussionContent.append(info);

  // answer
  const answer = document.createElement('p');
  const answerDelete = document.createElement('button');
  answerDelete.className = 'deleteBtn';
  answerDelete.textContent = '삭제';
  answerDelete.addEventListener('click', (e) => {
    console.log('delete button');
    li.remove();
  });

  answer.textContent = obj.answer ? '☑︎' : '☒';
  discussionAnswered.append(answer, answerDelete);
