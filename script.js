console.log(agoraStatesDiscussions)// index.html을 열어서 agoraStatesDiscussions 배열 요소를 확인하세요.
...
const convertToDiscussion = (obj) => {
  ...
  // 2. 콘텐츠 영역
  // 2-1 제목
  const contentTittle = document.createElement('h2')
  contentTittle.className ="discussion__title"
  ...
  console.log(contentInfo.textContent)
  discussionContent.append(contentTittle,contentInfo)
  titleAnchor.textContent = obj.title;
  titleAnchor.href = obj.url;

  contentTittle.append(titleAnchor)
  // 3. 체크박스 영역
...
const render = (element) => {
  ...
  for (let i = 0; i < agoraStatesDiscussions.length; i += 1) {
    // i번째 요소를 convertToDiscussion에 전달하여  ul에 append
