...
const convertToDiscussion = (obj) => {
  ...
  const avtImg = document.createElement('img'); // 이미지 불러오기
  ...
  info.textContent = `${obj.author} / ${new Date(obj.createAt).toLocaleTimeString()}`;
...
document.getElementById("btn").addEventListener("click", btnClick)
 ...
 let atText = document.getElementsByClassName("auName")[0];  // 이름 클래스 첫번째 불러오기
 
 let title2 = document.getElementsByClassName("title")[0];


function btnClick () {

  // console.log(atText.value);
  ...
  ul.prepend(convertToDiscussion(obj))  // prepend 맨앞에 추가
