...
const convertToDiscussion = (obj) => {
  ...
  contLink.href = obj.url;
  contLink.textContent = obj.title;
  const contTitle = document.createElement('h2');
  contTitle.className = "discussion__title";
  contTitle.append(contLink);

  const authorInfo = document.createElement('div');
  authorInfo.className = "discussion__information";
  // 날짜를 우리가 읽기 쉬운 형식으로 만들어주기 위해 new Date의 toLocaleString 메소드를 사용
  authorInfo.textContent = `${obj.author} / ${new Date(obj.createdAt).toLocaleString()}`;

  discussionContent.append(contTitle, authorInfo);
...
const formText = document.querySelector(".form__textbox > textarea");
