...
const convertToDiscussion = (obj) => {
  ...
  const discussTitleAnchor = document.createElement('a');
  discussTitleAnchor.textContent = obj.title;
  discussTitleAnchor.href = obj.url;
  ...
  discussTitle.append(discussTitleAnchor);
  discussionContent.append(discussTitle);  
  discussionContent.append(discussInfo);  

  // const answerTypeCheck = <i class="fa-solid fa-badge-check"></i>;
  if (obj.answer) {
    const discussAnswerType = document.createElement('p')
    discussAnswerType.textContent = '☑';
    discussionAnswered.append(discussAnswerType);
  } 
...
const enterTitle = document.querySelector('.form__input--title > input');
const enterContent = document.querySelector('.form__textbox > textarea');
const enterSubmit = document.querySelector('.form__submit > input');
// const enterCreated = new Date().toLocaleString();
...
enterForm.addEventListener('submit', eventForm);
function eventForm(event) {
  // console.log(event)
  event.preventDefault();
  alert('눌러요');
  newDisscuss.author = enterName.value;
  newDisscuss.title = enterTitle.value;
  agoraStatesDiscussions.unshift(newDisscuss);
