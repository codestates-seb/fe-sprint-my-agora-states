...
const convertToDiscussion = (obj) => {
 ...
 //////////////////////////////////////////////////////////
  ...
  contentInfo.textContent = `${obj.author} / ${new Date(obj.createdAt).toLocaleDateString()}`
...
form.addEventListener('submit', (event) => {    ///// input type="submit" , () = submit 했을때의 함수 
  event.preventDefault(); // **form요소는 submit 동작이 일어나면 자동으로 새로고침 => 새로 고침을 막기위한 preventDefault
// 하나의 객체를 만들어 convertToDiscussion함수에 넣어서 li로 만든 다음 ul 요소에 append
  const newdiscussion = {
    id: "unique value",
    createdAt: new Date(),
    title: title.value,
  ...
  ul.prepend(convertToDiscussion(newdiscussion)) // append의 경우 맨 뒤로 감 => prepend
  title.value = "";
  author.value = "";
  story.value = "";
})
...
// agoraStatesDiscussions 배열의 모든 데이터를 화면에 렌더링하는 함수입니다. ///// element -> ul ////////
