...
const lenAgoraData = agoraStatesDiscussions.length;
...
console.log(lenAgoraData);

// agoraStatesDiscussions 배열의 모든 데이터를 화면에 렌더링하는 함수입니다.
const render = (element) => {
  console.log(element);
  for (let i = 0; i < lenAgoraData; i += 1) {
    element.append(convertToDiscussion(localDiscussions[i]));
  }
  // 질문 추가시 작동
  for (let j = lenAgoraData; j < localDiscussions.length; j++){
    element.prepend(convertToDiscussion(localDiscussions[j]));
  }
  return;
};
