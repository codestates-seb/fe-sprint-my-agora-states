 ...
 */
const renderPagination = (totalPage, maxShowPage, page) => {
...
fetch(`${BASE_URL}/discussions`) //
  ...
  .then((json) => {
    data = json;
    page = 1; // 현재 페이지 인덱스
    totalPage = Math.ceil(json.length / PAGE_ITEM_NUMBERS); // 총 페이지 숫자

    // ul 요소에 agoraStatesDiscussions 배열의 모든 데이터를 화면에 렌더링합니다.
    render(ul, 0, PAGE_ITEM_NUMBERS, json); // 초기 화면 렌더링
    // 페이지네이션 렌더링 하기
    renderPagination(totalPage, 5, page);
  });
...
$form.addEventListener('submit', (e) => {
  ...
  page = 1; // 페이지 1로 변경
  totalPage = Math.ceil(data.length / PAGE_ITEM_NUMBERS); // 토탈 페이지 다시 계산

  // 맨 앞 페이지로 이동하고, 다시 렌더링
  render(ul, 0, PAGE_ITEM_NUMBERS, data); // 초기 화면 렌더링
  // 페이지네이션 다시 렌더링
  renderPagination(totalPage, 5, page);

  // 제출한 배열을 로컬 스토리지에 저장한다.
  localStorage.setItem('discussionList', JSON.stringify(data));
