...
const submitBtn = document
  ...
  .querySelector("input"); //submit버튼을 눌렀을 경우 이벤트 리스너 작성
submitBtn.addEventListener("click", (e) => {
  const questionName = document
    .querySelector(".form__input--name")
    .querySelector("input").value; // ".form__input--name"의 input에 적혀진 이름을 받아옴
  const questionTitle = document
    .querySelector(".form__input--title")
    .querySelector("input").value; // form__input--title의 input에 적힌 타이틀을 받아옴
  const questionContent = document
    .querySelector(".form__textbox")
    .querySelector("textarea").value; //".form__textbox"의 textarea에 적힌 질문내용을 받아옴
  ...
  localStorage.setItem("items", JSON.stringify(newProperty));
  arr.push(JSON.parse(localStorage.getItem("items"))); // 로컬스토리지의 정보를 뺴옴
