...
const li = document.querySelector(".discussion__container");

// 디스커션 추가기능
// submit을 누르면 addEventLister로 srcElement[0].value -> 를 agoraStatesDiscussions에 새로운 배열의 author에 할당
// submit을 누르면 addEventLister로  srcElement[1].value -> 를 agoraStatesDiscussions에 새로운 배열의 title에 할당
// submit을 누르면 addEventLister로  srcElement[2].value -> 를 agoraStatesDiscussions에 새로운 배열의 bodyHTML에 할당
// ul 안에 있는 li 다 지우고
// rander(ul)
...
function onLoginSubmit(event) {
  ...
  const now = `${year}-${month}-${day}T${time}Z`;
  discussionAdd.createdAt = now;
  discussionAdd.author = event.srcElement[0].value;
  discussionAdd.title = event.srcElement[1].value;
  discussionAdd.bodyHTML = event.srcElement[2].value;
  agoraStatesDiscussions.unshift(discussionAdd);
...
const convertToDiscussion = (obj) => {
  ...
  information.innerHTML = `${obj.author} / ${obj.createdAt}`;
