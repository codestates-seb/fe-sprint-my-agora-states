...
const render = (element, limit, pageNumber) => {
  //S2 MiniHack server 연결 추가
  let arr2;
  let arr = fetch("http://localhost:4000/discussions/", {
    method: "GET"
  })
  .then(res => res.json())
  .then(data => {
  ...
  // for (let i = limit * pageNumber - 10; i < limit * pageNumber; i += 1) {
  //   // element.append(convertToDiscussion(agoraStatesDiscussions[i]));
  //   element.append(convertToDiscussion(arr2[i]));
  // }
  // return;
};
...
setTimeout(render(ul, 10, 2), 3000);
...
form.addEventListener("submit", (event) => {
  ...
  event.preventDefault(); // 옛날에는 무조건 새로고침을 했어야함 -> submit을 하면 새로고침을 하기로 약속
  const obj = {
    id: "unique id",
    createdAt: new Date(),
    title: title.value,
  ...
  arr2.unshift(obj);
  const newDiscussion = convertToDiscussion(obj) //DOM으로 변경진행화
  localStorage.setItem('new agora', JSON.stringify(arr2));
