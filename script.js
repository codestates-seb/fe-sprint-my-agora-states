...
const convertToDiscussion = (obj) => {
 ...
 //2. 콘텐츠 영역
 // 2-1 질문의 큰 제목 나오게 만들기 
 const discussionTitle = document.createElement('h2'); 
 discussionTitle.className = "discussion__title";
 discussionContent.append(discussionTitle);

// 2-2 링크 (타이틀 누르면 연결되는 링크) - 제목은 a요소에 들어감
 ...
 contentInfo.className = "discussion__information"; // css가 적용이 안되서 class명 사용
 contentInfo.textContent = `${obj.author} / ${new Date(obj.createdAt).toLocaleString()}`  // 오전 00:00:00
 discussionContent.append(contentInfo);


 // 3.체크박스 - 답변여부에 따라 체크박스가 달라짐
...
form.addEventListener('submit', (event) => {
 ...
 //객체를 만들고
const obj ={
   id: "", //자동적으로 고유번호가 쌓일꺼라 임의로 넣어도 상관없음
   createdAt: new Date(),
   title: title.value,
 ...
 //객체를 converToDiscussion 에 넣어서 DOM으로 변환
 //이후 render함수에 넣어서 브라우저에 렌더링한다?
 ul.prepend(convertToDiscussion(obj));
 
 //글 작성이 완료되어 글을 올리면 작성칸은 공백이 됨
 title.value="";
 author.value="";
 textArea.value="";
})
