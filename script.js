...
const convertToDiscussion = (obj) => {
  ...
  // 1.이미지 파일 가져오기/ 아바타 영역  //obj는 convertToDiscussion의 매개변수 
  const avatarImg = document.createElement('img'); 
  avatarImg.src = obj.avatarUrl; // obj안에 있는 링크 
  avatarImg.alt = "avatar of" + obj.author; // obj안에 있는 사람 이름?
  avatarWrapper.append(avatarImg);

  // h2의 부모 요소인 "discussion__content"한테 
  // DOM으로 받아와서 append로 넣어준다 
  // 2.콘텐츠 영역
  // 2-1 제목
  const contentTitle = document.createElement('h2');
  const titleAnchor = document.createElement('a'); // h2안에 있음 
  contentTitle.className = 'discussion__title'
  titleAnchor.textContent = obj.title //  a태그가 질문내용 title은 질문 내용 목록을 넣어준다
  titleAnchor.href = obj.url;
  
  contentTitle.append(titleAnchor);// a태그는 h2에 넣어줌
  
  // 2-2 정보창 
  const contentInfo = document.createElement('div');
  contentInfo.textContent = `${obj.author} / ${new Date(obj.createdAt).toLocaleString()}`//시간 날짜 
