...
https://crowwan.github.io/fe-sprint-my-agora-states/
...
- [x] 간단한 로그인 기능
- [x] 로그인 된 유저만 디스커션 작성 가능
- [x] 현재 로그인 된 유저는 자신이 작성한 디스커션만 삭제 가능
- [x] 로그인 된 유저가 작성하지 않은 디스커션에 대해 answer하는 기능 (이미 답변이 달린 디스커션 제외)
- [x] 미리 주어진 태그들에 대해 필터링
- [x] 답변이 달리지 않은 디스커션 필터링
...
> 태그 필터링 디자인을 모바일이나 테블릿 화면에서 select를 이용하려 했지만, 생각보다 구현이 하기 어려워 스크롤하는 형태로 한 것이 아쉽습니다.

### 도움을 받고 싶은 부분

> 디자인에 소질이 없어 깃허브를 참고했는데, 하면서 css가 무너지는 경우가 많았습니다. 어찌저찌 만들긴 했는데 지금도 헷갈리는 부분이 많네요.
