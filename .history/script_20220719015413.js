...
const arrAskInputName = [];
const arrAskInputTitle = [];

function saveToAsk() {
  localStorage.setItem("toAskInputName", JSON.stringify(arrAskInputName));
  localStorage.setItem("toAskInputTitle", JSON.stringify(arrAskInputTitle));
}
...
function handleToDoSubmit(e) {  // 이벤트 발생시 작동할 리스너의 함수 [텍스트 입력 내용을 변수에 저장하고, 초기화함 + 화면에 보여주는기능(함수로)]
  ...
  arrAskInputName.push(newInputName); //
  arrAskInputTitle.push(newInputTitle); //
...
const savedToAsks = localStorage.getItem("toAskInputName");
...
if (savedToAsks) {
  const parsedToAsk = JSON.parse(savedToAsks);
  console.log(parsedToAsk);
  parsedToAsk.forEach(paintToAsk());
  console.log(parsedToAsk);
}