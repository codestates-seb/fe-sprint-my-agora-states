...
const AskForm = document.querySelector(".form");   // 작성폼 전체
...
AskForm.addEventListener("submit", handleToDoSubmit); // 작성폼 제출시 함수 작동하는 이벤트 리스너 생성
...
function handleToDoSubmit(e) {  // 이벤트 발생시 작동할 리스너의 함수 []
  ...
  const newdiscussion = convertToDiscussion(obj); // 새로운 객체가 삽입된 상태의 더미데이터를 변수에 저장
