...
console.log(agoraStatesDiscussions.author);
...
const convertToDiscussion = (obj) => {
  ...
  avatarImg.src = agoraStatesDiscussions[i].avatarUrl; // img 요소의 속성 추가  ?? setAttribute 안쓰고?
  ...
  const discussionContentTitleAnchor = document.createElement("a"); // a 요소 생성
    discussionContentTitleAnchor.href = agoraStatesDiscussions[i].url; // a 요소 속성 href 추가
    discussionContentTitleAnchor.textContent = agoraStatesDiscussions[i].title;  // a 요소 내용 추가
  ...
  const dicussionContentInformation = document.createElement("div"); // div 요소 생성
    ...
    dicussionContentInformation.textContent = agoraStatesDiscussions[i].author + " / " + agoraStatesDiscussions[i].createdAt; // div 요소 내용 작성자, 작성시간 추가
  ...
  const discussionAnswered = document.createElement("div");  // div 요소 생성
    ...
    discussionAnswered.textContent = isAnswerNull(agoraStatesDiscussions[i].answer); // answer 데이터 판별 결과를 내용에 넣기
