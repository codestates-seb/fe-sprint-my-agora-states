...
AskForm.addEventListener("submit", (e) => {   // // 작성폼 제출시 함수 작동하는 이벤트 리스너 생성 및 함수 실행 []
  ...
  saveInputObj(newObj); // 새로 추가되서 업데이트 된 를 string으로 변환해서 로컬 스토리지에 저장
...
function saveInputObj(arr) {
  arrInputObj.push(arr);
  localStorage.setItem("arrInputObj", JSON.stringify(arrInputObj)); //  JSON.stringify로 stirng으로 변환(로컬 스토리지는 string 형태로만 저장 가능)
}
